  ...
  return {
    ...
    listActivities(employeeId) {
      ...
      return store.activities
        .filter(item => item.status === 'published' && isEligible(employee, item))
        .map(item => ({
          ...publicActivity(item),
          sessions: store.sessions.filter(session => session.activityId === item.id).map(session => publicSession(store, session, employeeId))
        }));
    },
    ...
    submitEvidence(enrollmentId, input) {
      ...
      const enrollment = store.enrollments.find(item => item.id === enrollmentId);
      if (!enrollment) throw new CareerStoreError('NOT_FOUND', 'Запись не найдена');
      if (!['enrolled', 'confirmed', 'in_progress'].includes(enrollment.status)) throw new CareerStoreError('CONFLICT', 'Evidence нельзя отправить в текущем статусе');
