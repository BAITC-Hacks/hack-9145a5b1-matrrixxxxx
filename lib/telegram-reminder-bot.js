...
function createTelegramReminderBot({ storePath, token, botUsername = '', telegram, now = () => Date.now() }) {
  ...
  function eventKeyboard(event) {
    const rows = [];
    if (event.status !== 'confirmed') rows.push([{ text: '✓ Подтверждаю участие', callback_data: `confirm:${event.id}` }]);
    rows.push([
      { text: 'В календарь', callback_data: `calendar:${event.id}` },
      { text: 'Отменить запись', callback_data: `cancel:${event.id}` }
    ]);
  ...
  async function handleStart(message, linkCode) {
    ...
    if (!event) {
      return api('sendMessage', {
        chat_id: chatId,
        text: 'Эта ссылка уже использована или истекла. Вернитесь в Career Quest и создайте новую запись.',
        reply_markup: dashboardKeyboard()
      });
    }
