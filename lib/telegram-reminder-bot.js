...
function createTelegramReminderBot({ storePath, token, botUsername = '', telegram, now = () => Date.now(), onConfirm, onCancel }) {
  ...
  function hasUsableLink(event, linkCode) {
    if (event.linkToken !== linkCode || typeof event.linkExpiresAt !== 'string') return false;
    const expiresAt = Date.parse(event.linkExpiresAt);
