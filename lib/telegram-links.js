...
function createTelegramLinks({ storePath, careerStore, config, now = () => Date.now() }) {
  ...
  function create(actor, body) {
    ...
    const context = careerStore.getEnrollmentContext(body.careerEnrollmentId.trim());
    ...
    if (!['enrolled', 'confirmed'].includes(careerEnrollment.status)) {
      throw new CareerStoreError('CONFLICT', 'Telegram можно подключить только к подтверждённой записи на сессию');
    }
    const session = context.session;
    if (!session || session.status !== 'scheduled' || ['cancelled', 'closed'].includes(session.availability)) {
    ...
    if (!activity?.id || !activity.title || !bootstrap.employee) {
